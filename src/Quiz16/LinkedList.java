...
// ADD MORE METHODS HERE (like for assigned CS211 work):
    ...
    public boolean isPerfectMirror() {
        if(size % 2 == 1 || size < 1) { return false; } // if it is odd or size 0 then false
        for(int i = 0; i < size/2; i++) { // assuming the list is even uses compareTo
            if (nodeAt(i).data.compareTo(nodeAt(size-i - 1).data) == 0) {
                break;
            } else return false;
        }
    ...
    public void undoMirror() {
        ...
        if (isPerfectMirror()) {
            for(int i=0; i<initialSize/2; i++) { // since size is dynamic it will run half the list
                remove(size-1);
            }
        }
    }
